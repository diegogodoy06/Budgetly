...
const CreateRuleModal: React.FC<CreateRuleModalProps> = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState<AutomationRuleFormData>({
    ...
    conditions: [{ field: 'description', condition_type: 'description_contains', value: '' }],
    actions: [{ action_type: 'set_category', value: '' }]
  });
  ...
  const loadOptions = async () => {
    try {
      console.log('🔄 Carregando opções para regras...');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Nova Regra de Automação</h2>
          <button
            onClick={onClose}
